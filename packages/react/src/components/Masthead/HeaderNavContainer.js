...
const HeaderNavContainer = ({ children }) => {
  ...
  const paginateLeft = useCallback(() => {
    headerNavContainerRef.current.scrollLeft =
      Math.max(headerNavContainerRef.current.scrollLeft -
        headerNavContainerRef.current.offsetWidth, 0);
  }, [])

  const paginateRight = useCallback(() => {
    headerNavContainerRef.current.scrollLeft =
      Math.min(headerNavContainerRef.current.scrollLeft +
        headerNavContainerRef.current.offsetWidth, document.querySelector(".bx--header__nav").offsetWidth
  ...
  useEffect(() => {
    const { current: headerNavContainerNode } = headerNavContainerRef;
    setIO(new IntersectionObserver(records => {
